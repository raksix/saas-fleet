...
export async function GET() {
  ...
  const items = await prisma.planningpoker.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.planningpoker.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "planningpoker.create", targetType: "planningpoker", targetId: item.id,
    },
  });
