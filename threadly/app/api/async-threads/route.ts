...
export async function GET() {
  ...
  const items = await prisma.asyncthreads.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.asyncthreads.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "asyncthreads.create", targetType: "asyncthreads", targetId: item.id,
    },
  });
