...
export async function GET() {
  ...
  const items = await prisma.livecoaching.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.livecoaching.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "livecoaching.create", targetType: "livecoaching", targetId: item.id,
    },
  });
