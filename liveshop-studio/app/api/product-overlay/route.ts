...
export async function GET() {
  ...
  const items = await prisma.productoverlay.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.productoverlay.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "productoverlay.create", targetType: "productoverlay", targetId: item.id,
    },
  });
