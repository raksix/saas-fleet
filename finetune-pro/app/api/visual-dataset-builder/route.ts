...
export async function GET() {
  ...
  const items = await prisma.visualdatasetbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualdatasetbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualdatasetbuilder.create", targetType: "visualdatasetbuilder", targetId: item.id,
    },
  });
