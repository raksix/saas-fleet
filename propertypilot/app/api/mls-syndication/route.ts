...
export async function GET() {
  ...
  const items = await prisma.mlssyndication.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.mlssyndication.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "mlssyndication.create", targetType: "mlssyndication", targetId: item.id,
    },
  });
