...
export async function GET() {
  ...
  const items = await prisma.customplayer.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.customplayer.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "customplayer.create", targetType: "customplayer", targetId: item.id,
    },
  });
