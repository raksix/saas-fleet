...
export async function GET() {
  ...
  const items = await prisma.projectbudgets.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.projectbudgets.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "projectbudgets.create", targetType: "projectbudgets", targetId: item.id,
    },
  });
