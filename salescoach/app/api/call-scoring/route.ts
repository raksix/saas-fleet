...
export async function GET() {
  ...
  const items = await prisma.callscoring.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.callscoring.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "callscoring.create", targetType: "callscoring", targetId: item.id,
    },
  });
