...
export async function GET() {
  ...
  const items = await prisma.dark-webscan.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.dark-webscan.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "dark-webscan.create", targetType: "dark-webscan", targetId: item.id,
    },
  });
