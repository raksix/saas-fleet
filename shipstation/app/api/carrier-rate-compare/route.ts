...
export async function GET() {
  ...
  const items = await prisma.carrierratecompare.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.carrierratecompare.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "carrierratecompare.create", targetType: "carrierratecompare", targetId: item.id,
    },
  });
