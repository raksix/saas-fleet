...
export async function GET() {
  ...
  const items = await prisma.cross-sourcededup.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cross-sourcededup.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cross-sourcededup.create", targetType: "cross-sourcededup", targetId: item.id,
    },
  });
