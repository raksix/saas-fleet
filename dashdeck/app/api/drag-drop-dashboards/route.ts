...
export async function GET() {
  ...
  const items = await prisma.drag-dropdashboards.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.drag-dropdashboards.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "drag-dropdashboards.create", targetType: "drag-dropdashboards", targetId: item.id,
    },
  });
