...
export async function GET() {
  ...
  const items = await prisma.voice-trainedgeneration.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.voice-trainedgeneration.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "voice-trainedgeneration.create", targetType: "voice-trainedgeneration", targetId: item.id,
    },
  });
