...
export async function GET() {
  ...
  const items = await prisma.one-lineembed.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.one-lineembed.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "one-lineembed.create", targetType: "one-lineembed", targetId: item.id,
    },
  });
