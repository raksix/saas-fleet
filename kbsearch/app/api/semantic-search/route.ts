...
export async function GET() {
  ...
  const items = await prisma.semanticsearch.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.semanticsearch.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "semanticsearch.create", targetType: "semanticsearch", targetId: item.id,
    },
  });
