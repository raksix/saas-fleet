...
export async function GET() {
  ...
  const items = await prisma.drag-dropui.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.drag-dropui.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "drag-dropui.create", targetType: "drag-dropui", targetId: item.id,
    },
  });
