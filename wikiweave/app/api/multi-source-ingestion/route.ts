...
export async function GET() {
  ...
  const items = await prisma.multi-sourceingestion.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-sourceingestion.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-sourceingestion.create", targetType: "multi-sourceingestion", targetId: item.id,
    },
  });
