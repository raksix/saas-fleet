...
export async function GET() {
  ...
  const items = await prisma.multi-language.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-language.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-language.create", targetType: "multi-language", targetId: item.id,
    },
  });
