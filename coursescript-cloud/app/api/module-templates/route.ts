...
export async function GET() {
  ...
  const items = await prisma.moduletemplates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.moduletemplates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "moduletemplates.create", targetType: "moduletemplates", targetId: item.id,
    },
  });
