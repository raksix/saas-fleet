...
export async function GET() {
  ...
  const items = await prisma.codereviewqueue.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.codereviewqueue.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "codereviewqueue.create", targetType: "codereviewqueue", targetId: item.id,
    },
  });
