...
export async function GET() {
  ...
  const items = await prisma.stylelibrary.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.stylelibrary.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "stylelibrary.create", targetType: "stylelibrary", targetId: item.id,
    },
  });
