...
export async function GET() {
  ...
  const items = await prisma.hooktemplates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.hooktemplates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "hooktemplates.create", targetType: "hooktemplates", targetId: item.id,
    },
  });
