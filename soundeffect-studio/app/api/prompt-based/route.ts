...
export async function GET() {
  ...
  const items = await prisma.prompt-based.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.prompt-based.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "prompt-based.create", targetType: "prompt-based", targetId: item.id,
    },
  });
