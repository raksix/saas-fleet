...
export async function GET() {
  ...
  const items = await prisma.replaycapability.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.replaycapability.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "replaycapability.create", targetType: "replaycapability", targetId: item.id,
    },
  });
