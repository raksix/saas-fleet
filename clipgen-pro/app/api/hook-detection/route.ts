...
export async function GET() {
  ...
  const items = await prisma.hookdetection.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.hookdetection.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "hookdetection.create", targetType: "hookdetection", targetId: item.id,
    },
  });
