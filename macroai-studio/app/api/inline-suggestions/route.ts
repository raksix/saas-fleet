...
export async function GET() {
  ...
  const items = await prisma.inlinesuggestions.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.inlinesuggestions.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "inlinesuggestions.create", targetType: "inlinesuggestions", targetId: item.id,
    },
  });
