...
export async function GET() {
  ...
  const items = await prisma.liveratefeed.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.liveratefeed.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "liveratefeed.create", targetType: "liveratefeed", targetId: item.id,
    },
  });
