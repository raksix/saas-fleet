...
export async function GET() {
  ...
  const items = await prisma.themeextraction.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.themeextraction.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "themeextraction.create", targetType: "themeextraction", targetId: item.id,
    },
  });
