...
export async function GET() {
  ...
  const items = await prisma.timestampresults.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.timestampresults.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "timestampresults.create", targetType: "timestampresults", targetId: item.id,
    },
  });
