...
export async function GET() {
  ...
  const items = await prisma.curationengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.curationengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "curationengine.create", targetType: "curationengine", targetId: item.id,
    },
  });
