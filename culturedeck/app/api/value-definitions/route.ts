...
export async function GET() {
  ...
  const items = await prisma.valuedefinitions.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.valuedefinitions.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "valuedefinitions.create", targetType: "valuedefinitions", targetId: item.id,
    },
  });
