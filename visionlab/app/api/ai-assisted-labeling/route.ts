...
export async function GET() {
  ...
  const items = await prisma.ai-assistedlabeling.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.ai-assistedlabeling.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "ai-assistedlabeling.create", targetType: "ai-assistedlabeling", targetId: item.id,
    },
  });
