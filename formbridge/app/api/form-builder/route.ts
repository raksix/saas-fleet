...
export async function GET() {
  ...
  const items = await prisma.formbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.formbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "formbuilder.create", targetType: "formbuilder", targetId: item.id,
    },
  });
