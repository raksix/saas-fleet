...
export async function GET() {
  ...
  const items = await prisma.360reviewengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.360reviewengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "360reviewengine.create", targetType: "360reviewengine", targetId: item.id,
    },
  });
