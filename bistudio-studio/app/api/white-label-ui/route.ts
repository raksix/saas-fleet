...
export async function GET() {
  ...
  const items = await prisma.white-labelui.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.white-labelui.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "white-labelui.create", targetType: "white-labelui", targetId: item.id,
    },
  });
