...
export async function GET() {
  ...
  const items = await prisma.rmagenerator.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.rmagenerator.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "rmagenerator.create", targetType: "rmagenerator", targetId: item.id,
    },
  });
