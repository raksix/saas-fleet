...
export async function GET() {
  ...
  const items = await prisma.visualbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualbuilder.create", targetType: "visualbuilder", targetId: item.id,
    },
  });
