...
export async function GET() {
  ...
  const items = await prisma.one-clickbackup.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.one-clickbackup.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "one-clickbackup.create", targetType: "one-clickbackup", targetId: item.id,
    },
  });
