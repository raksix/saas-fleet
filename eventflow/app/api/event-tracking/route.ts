...
export async function GET() {
  ...
  const items = await prisma.eventtracking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.eventtracking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "eventtracking.create", targetType: "eventtracking", targetId: item.id,
    },
  });
