...
export async function GET() {
  ...
  const items = await prisma.compositehealthscore.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.compositehealthscore.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "compositehealthscore.create", targetType: "compositehealthscore", targetId: item.id,
    },
  });
