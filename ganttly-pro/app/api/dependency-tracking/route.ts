...
export async function GET() {
  ...
  const items = await prisma.dependencytracking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.dependencytracking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "dependencytracking.create", targetType: "dependencytracking", targetId: item.id,
    },
  });
