...
export async function GET() {
  ...
  const items = await prisma.multi-classclassification.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-classclassification.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-classclassification.create", targetType: "multi-classclassification", targetId: item.id,
    },
  });
