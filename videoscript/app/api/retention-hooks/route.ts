...
export async function GET() {
  ...
  const items = await prisma.retentionhooks.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.retentionhooks.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "retentionhooks.create", targetType: "retentionhooks", targetId: item.id,
    },
  });
