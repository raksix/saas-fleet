...
export async function GET() {
  ...
  const items = await prisma.curatedfeeds.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.curatedfeeds.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "curatedfeeds.create", targetType: "curatedfeeds", targetId: item.id,
    },
  });
