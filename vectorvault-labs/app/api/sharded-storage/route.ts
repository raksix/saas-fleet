...
export async function GET() {
  ...
  const items = await prisma.shardedstorage.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.shardedstorage.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "shardedstorage.create", targetType: "shardedstorage", targetId: item.id,
    },
  });
