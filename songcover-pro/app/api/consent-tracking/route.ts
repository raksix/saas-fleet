...
export async function GET() {
  ...
  const items = await prisma.consenttracking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.consenttracking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "consenttracking.create", targetType: "consenttracking", targetId: item.id,
    },
  });
