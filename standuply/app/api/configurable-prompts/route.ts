...
export async function GET() {
  ...
  const items = await prisma.configurableprompts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.configurableprompts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "configurableprompts.create", targetType: "configurableprompts", targetId: item.id,
    },
  });
