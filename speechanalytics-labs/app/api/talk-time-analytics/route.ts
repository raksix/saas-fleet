...
export async function GET() {
  ...
  const items = await prisma.talk-timeanalytics.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.talk-timeanalytics.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "talk-timeanalytics.create", targetType: "talk-timeanalytics", targetId: item.id,
    },
  });
