...
export async function GET() {
  ...
  const items = await prisma.sprintplanning.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.sprintplanning.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "sprintplanning.create", targetType: "sprintplanning", targetId: item.id,
    },
  });
