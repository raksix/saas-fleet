...
export async function GET() {
  ...
  const items = await prisma.unifiedcampaigncanvas.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.unifiedcampaigncanvas.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "unifiedcampaigncanvas.create", targetType: "unifiedcampaigncanvas", targetId: item.id,
    },
  });
