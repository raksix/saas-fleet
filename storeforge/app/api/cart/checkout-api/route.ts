...
export async function GET() {
  ...
  const items = await prisma.cart/checkoutapi.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cart/checkoutapi.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cart/checkoutapi.create", targetType: "cart/checkoutapi", targetId: item.id,
    },
  });
