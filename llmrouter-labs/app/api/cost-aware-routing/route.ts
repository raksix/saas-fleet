...
export async function GET() {
  ...
  const items = await prisma.cost-awarerouting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cost-awarerouting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cost-awarerouting.create", targetType: "cost-awarerouting", targetId: item.id,
    },
  });
