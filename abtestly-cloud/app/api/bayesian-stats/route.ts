...
export async function GET() {
  ...
  const items = await prisma.bayesianstats.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.bayesianstats.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "bayesianstats.create", targetType: "bayesianstats", targetId: item.id,
    },
  });
