...
export async function GET() {
  ...
  const items = await prisma.rfm+behavioralsegments.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.rfm+behavioralsegments.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "rfm+behavioralsegments.create", targetType: "rfm+behavioralsegments", targetId: item.id,
    },
  });
