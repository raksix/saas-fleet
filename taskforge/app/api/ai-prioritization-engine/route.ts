...
export async function GET() {
  ...
  const items = await prisma.aiprioritizationengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.aiprioritizationengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "aiprioritizationengine.create", targetType: "aiprioritizationengine", targetId: item.id,
    },
  });
