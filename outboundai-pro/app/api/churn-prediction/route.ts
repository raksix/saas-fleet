...
export async function GET() {
  ...
  const items = await prisma.churnprediction.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.churnprediction.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "churnprediction.create", targetType: "churnprediction", targetId: item.id,
    },
  });
