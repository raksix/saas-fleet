...
export async function GET() {
  ...
  const items = await prisma.voicemailtranscription.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.voicemailtranscription.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "voicemailtranscription.create", targetType: "voicemailtranscription", targetId: item.id,
    },
  });
