...
export async function GET() {
  ...
  const items = await prisma.triagequeue.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.triagequeue.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "triagequeue.create", targetType: "triagequeue", targetId: item.id,
    },
  });
