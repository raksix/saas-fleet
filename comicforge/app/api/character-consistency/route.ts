...
export async function GET() {
  ...
  const items = await prisma.characterconsistency.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.characterconsistency.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "characterconsistency.create", targetType: "characterconsistency", targetId: item.id,
    },
  });
