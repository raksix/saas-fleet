...
export async function GET() {
  ...
  const items = await prisma.serverlessruntime.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.serverlessruntime.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "serverlessruntime.create", targetType: "serverlessruntime", targetId: item.id,
    },
  });
