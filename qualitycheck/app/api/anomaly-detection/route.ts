...
export async function GET() {
  ...
  const items = await prisma.anomalydetection.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.anomalydetection.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "anomalydetection.create", targetType: "anomalydetection", targetId: item.id,
    },
  });
