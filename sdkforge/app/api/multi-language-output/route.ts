...
export async function GET() {
  ...
  const items = await prisma.multi-languageoutput.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-languageoutput.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-languageoutput.create", targetType: "multi-languageoutput", targetId: item.id,
    },
  });
