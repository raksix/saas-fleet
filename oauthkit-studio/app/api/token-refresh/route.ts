...
export async function GET() {
  ...
  const items = await prisma.tokenrefresh.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.tokenrefresh.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "tokenrefresh.create", targetType: "tokenrefresh", targetId: item.id,
    },
  });
