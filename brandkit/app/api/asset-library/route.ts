...
export async function GET() {
  ...
  const items = await prisma.assetlibrary.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.assetlibrary.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "assetlibrary.create", targetType: "assetlibrary", targetId: item.id,
    },
  });
