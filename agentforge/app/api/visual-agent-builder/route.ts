...
export async function GET() {
  ...
  const items = await prisma.visualagentbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualagentbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualagentbuilder.create", targetType: "visualagentbuilder", targetId: item.id,
    },
  });
