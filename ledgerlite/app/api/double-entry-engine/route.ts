...
export async function GET() {
  ...
  const items = await prisma.double-entryengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.double-entryengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "double-entryengine.create", targetType: "double-entryengine", targetId: item.id,
    },
  });
