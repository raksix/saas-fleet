...
export async function GET() {
  ...
  const items = await prisma.recurring+usagebilling.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.recurring+usagebilling.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "recurring+usagebilling.create", targetType: "recurring+usagebilling", targetId: item.id,
    },
  });
