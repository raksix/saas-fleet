...
export async function GET() {
  ...
  const items = await prisma.multi-countryengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-countryengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-countryengine.create", targetType: "multi-countryengine", targetId: item.id,
    },
  });
