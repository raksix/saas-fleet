...
export async function GET() {
  ...
  const items = await prisma.simulatedattacks.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.simulatedattacks.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "simulatedattacks.create", targetType: "simulatedattacks", targetId: item.id,
    },
  });
