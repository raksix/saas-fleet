...
export async function GET() {
  ...
  const items = await prisma.100+languages.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.100+languages.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "100+languages.create", targetType: "100+languages", targetId: item.id,
    },
  });
