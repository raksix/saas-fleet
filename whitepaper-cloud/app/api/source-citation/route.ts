...
export async function GET() {
  ...
  const items = await prisma.sourcecitation.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.sourcecitation.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "sourcecitation.create", targetType: "sourcecitation", targetId: item.id,
    },
  });
