...
export async function GET() {
  ...
  const items = await prisma.bankfeedingest.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.bankfeedingest.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "bankfeedingest.create", targetType: "bankfeedingest", targetId: item.id,
    },
  });
