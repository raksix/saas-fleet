...
export async function GET() {
  ...
  const items = await prisma.real-timetranscription.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.real-timetranscription.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "real-timetranscription.create", targetType: "real-timetranscription", targetId: item.id,
    },
  });
