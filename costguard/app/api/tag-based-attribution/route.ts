...
export async function GET() {
  ...
  const items = await prisma.tag-basedattribution.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.tag-basedattribution.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "tag-basedattribution.create", targetType: "tag-basedattribution", targetId: item.id,
    },
  });
