...
export async function GET() {
  ...
  const items = await prisma.glossaryenforcement.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.glossaryenforcement.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "glossaryenforcement.create", targetType: "glossaryenforcement", targetId: item.id,
    },
  });
