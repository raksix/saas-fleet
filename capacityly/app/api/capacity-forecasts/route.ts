...
export async function GET() {
  ...
  const items = await prisma.capacityforecasts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.capacityforecasts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "capacityforecasts.create", targetType: "capacityforecasts", targetId: item.id,
    },
  });
