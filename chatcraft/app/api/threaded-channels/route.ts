...
export async function GET() {
  ...
  const items = await prisma.threadedchannels.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.threadedchannels.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "threadedchannels.create", targetType: "threadedchannels", targetId: item.id,
    },
  });
