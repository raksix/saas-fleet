...
export async function GET() {
  ...
  const items = await prisma.cohortretention.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cohortretention.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cohortretention.create", targetType: "cohortretention", targetId: item.id,
    },
  });
