...
export async function GET() {
  ...
  const items = await prisma.assetdiscovery.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.assetdiscovery.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "assetdiscovery.create", targetType: "assetdiscovery", targetId: item.id,
    },
  });
