...
export async function GET() {
  ...
  const items = await prisma.multi-provider.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-provider.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-provider.create", targetType: "multi-provider", targetId: item.id,
    },
  });
