...
export async function GET() {
  ...
  const items = await prisma.real-timestream.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.real-timestream.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "real-timestream.create", targetType: "real-timestream", targetId: item.id,
    },
  });
