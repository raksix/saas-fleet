...
export async function GET() {
  ...
  const items = await prisma.apikey+oauth.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.apikey+oauth.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "apikey+oauth.create", targetType: "apikey+oauth", targetId: item.id,
    },
  });
