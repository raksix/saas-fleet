...
export async function GET() {
  ...
  const items = await prisma.alertrouting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.alertrouting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "alertrouting.create", targetType: "alertrouting", targetId: item.id,
    },
  });
