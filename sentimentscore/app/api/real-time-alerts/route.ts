...
export async function GET() {
  ...
  const items = await prisma.real-timealerts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.real-timealerts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "real-timealerts.create", targetType: "real-timealerts", targetId: item.id,
    },
  });
