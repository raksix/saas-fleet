...
export async function GET() {
  ...
  const items = await prisma.webrecorder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.webrecorder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "webrecorder.create", targetType: "webrecorder", targetId: item.id,
    },
  });
