...
export async function GET() {
  ...
  const items = await prisma.speed-to-leadtext.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.speed-to-leadtext.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "speed-to-leadtext.create", targetType: "speed-to-leadtext", targetId: item.id,
    },
  });
