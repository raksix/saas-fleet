...
export async function GET() {
  ...
  const items = await prisma.affiliaterecruitment.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.affiliaterecruitment.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "affiliaterecruitment.create", targetType: "affiliaterecruitment", targetId: item.id,
    },
  });
