...
export async function GET() {
  ...
  const items = await prisma.image-to-video.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.image-to-video.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "image-to-video.create", targetType: "image-to-video", targetId: item.id,
    },
  });
