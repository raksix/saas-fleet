...
export async function GET() {
  ...
  const items = await prisma.bundlebuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.bundlebuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "bundlebuilder.create", targetType: "bundlebuilder", targetId: item.id,
    },
  });
