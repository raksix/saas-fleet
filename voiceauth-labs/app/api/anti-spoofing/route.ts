...
export async function GET() {
  ...
  const items = await prisma.anti-spoofing.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.anti-spoofing.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "anti-spoofing.create", targetType: "anti-spoofing", targetId: item.id,
    },
  });
