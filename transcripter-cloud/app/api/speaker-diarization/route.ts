...
export async function GET() {
  ...
  const items = await prisma.speakerdiarization.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.speakerdiarization.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "speakerdiarization.create", targetType: "speakerdiarization", targetId: item.id,
    },
  });
