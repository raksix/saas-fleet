...
export async function GET() {
  ...
  const items = await prisma.storyframeworks.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.storyframeworks.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "storyframeworks.create", targetType: "storyframeworks", targetId: item.id,
    },
  });
