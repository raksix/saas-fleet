...
export async function GET() {
  ...
  const items = await prisma.gitscanning.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.gitscanning.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "gitscanning.create", targetType: "gitscanning", targetId: item.id,
    },
  });
