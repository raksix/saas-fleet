...
export async function GET() {
  ...
  const items = await prisma.focus-timeauto-blocking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.focus-timeauto-blocking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "focus-timeauto-blocking.create", targetType: "focus-timeauto-blocking", targetId: item.id,
    },
  });
