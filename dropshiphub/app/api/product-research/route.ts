...
export async function GET() {
  ...
  const items = await prisma.productresearch.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.productresearch.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "productresearch.create", targetType: "productresearch", targetId: item.id,
    },
  });
