...
export async function GET() {
  ...
  const items = await prisma.cryptographicverify.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cryptographicverify.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cryptographicverify.create", targetType: "cryptographicverify", targetId: item.id,
    },
  });
