...
export async function GET() {
  ...
  const items = await prisma.multi-framework.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-framework.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-framework.create", targetType: "multi-framework", targetId: item.id,
    },
  });
