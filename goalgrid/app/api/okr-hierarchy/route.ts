...
export async function GET() {
  ...
  const items = await prisma.okrhierarchy.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.okrhierarchy.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "okrhierarchy.create", targetType: "okrhierarchy", targetId: item.id,
    },
  });
