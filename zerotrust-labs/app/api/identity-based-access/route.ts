...
export async function GET() {
  ...
  const items = await prisma.identity-basedaccess.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.identity-basedaccess.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "identity-basedaccess.create", targetType: "identity-basedaccess", targetId: item.id,
    },
  });
