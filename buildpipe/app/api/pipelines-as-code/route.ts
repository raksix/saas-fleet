...
export async function GET() {
  ...
  const items = await prisma.pipelines-as-code.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.pipelines-as-code.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "pipelines-as-code.create", targetType: "pipelines-as-code", targetId: item.id,
    },
  });
