...
export async function GET() {
  ...
  const items = await prisma.smartrouting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.smartrouting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "smartrouting.create", targetType: "smartrouting", targetId: item.id,
    },
  });
