...
export async function GET() {
  ...
  const items = await prisma.programbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.programbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "programbuilder.create", targetType: "programbuilder", targetId: item.id,
    },
  });
