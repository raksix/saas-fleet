...
export async function GET() {
  ...
  const items = await prisma.policyengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.policyengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "policyengine.create", targetType: "policyengine", targetId: item.id,
    },
  });
