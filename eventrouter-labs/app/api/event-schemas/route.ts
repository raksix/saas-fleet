...
export async function GET() {
  ...
  const items = await prisma.eventschemas.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.eventschemas.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "eventschemas.create", targetType: "eventschemas", targetId: item.id,
    },
  });
