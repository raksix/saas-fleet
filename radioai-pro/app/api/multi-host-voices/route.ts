...
export async function GET() {
  ...
  const items = await prisma.multi-hostvoices.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-hostvoices.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-hostvoices.create", targetType: "multi-hostvoices", targetId: item.id,
    },
  });
