...
export async function GET() {
  ...
  const items = await prisma.multi-modelbenchmarks.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-modelbenchmarks.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-modelbenchmarks.create", targetType: "multi-modelbenchmarks", targetId: item.id,
    },
  });
