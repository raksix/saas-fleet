...
export async function GET() {
  ...
  const items = await prisma.wsjfscoring.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.wsjfscoring.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "wsjfscoring.create", targetType: "wsjfscoring", targetId: item.id,
    },
  });
