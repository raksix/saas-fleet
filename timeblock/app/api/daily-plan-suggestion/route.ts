...
export async function GET() {
  ...
  const items = await prisma.dailyplansuggestion.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.dailyplansuggestion.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "dailyplansuggestion.create", targetType: "dailyplansuggestion", targetId: item.id,
    },
  });
