...
export async function GET() {
  ...
  const items = await prisma.multi-stepfunnels.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-stepfunnels.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-stepfunnels.create", targetType: "multi-stepfunnels", targetId: item.id,
    },
  });
