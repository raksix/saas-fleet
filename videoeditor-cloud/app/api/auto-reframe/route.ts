...
export async function GET() {
  ...
  const items = await prisma.auto-reframe.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-reframe.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-reframe.create", targetType: "auto-reframe", targetId: item.id,
    },
  });
