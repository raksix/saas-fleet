...
export async function GET() {
  ...
  const items = await prisma.vendoronboarding.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.vendoronboarding.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "vendoronboarding.create", targetType: "vendoronboarding", targetId: item.id,
    },
  });
