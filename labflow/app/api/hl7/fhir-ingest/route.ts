...
export async function GET() {
  ...
  const items = await prisma.hl7/fhiringest.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.hl7/fhiringest.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "hl7/fhiringest.create", targetType: "hl7/fhiringest", targetId: item.id,
    },
  });
