...
export async function GET() {
  ...
  const items = await prisma.200+connectors.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.200+connectors.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "200+connectors.create", targetType: "200+connectors", targetId: item.id,
    },
  });
