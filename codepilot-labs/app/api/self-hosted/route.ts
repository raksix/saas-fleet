...
export async function GET() {
  ...
  const items = await prisma.self-hosted.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.self-hosted.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "self-hosted.create", targetType: "self-hosted", targetId: item.id,
    },
  });
