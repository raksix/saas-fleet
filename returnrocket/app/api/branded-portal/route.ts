...
export async function GET() {
  ...
  const items = await prisma.brandedportal.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.brandedportal.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "brandedportal.create", targetType: "brandedportal", targetId: item.id,
    },
  });
