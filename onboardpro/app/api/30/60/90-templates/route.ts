...
export async function GET() {
  ...
  const items = await prisma.30/60/90templates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.30/60/90templates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "30/60/90templates.create", targetType: "30/60/90templates", targetId: item.id,
    },
  });
