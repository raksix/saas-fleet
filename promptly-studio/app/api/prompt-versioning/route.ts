...
export async function GET() {
  ...
  const items = await prisma.promptversioning.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.promptversioning.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "promptversioning.create", targetType: "promptversioning", targetId: item.id,
    },
  });
