...
export async function GET() {
  ...
  const items = await prisma.points+tiers.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.points+tiers.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "points+tiers.create", targetType: "points+tiers", targetId: item.id,
    },
  });
