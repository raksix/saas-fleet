...
export async function GET() {
  ...
  const items = await prisma.brand-voicetraining.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.brand-voicetraining.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "brand-voicetraining.create", targetType: "brand-voicetraining", targetId: item.id,
    },
  });
