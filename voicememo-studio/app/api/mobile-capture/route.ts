...
export async function GET() {
  ...
  const items = await prisma.mobilecapture.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.mobilecapture.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "mobilecapture.create", targetType: "mobilecapture", targetId: item.id,
    },
  });
