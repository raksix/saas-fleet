...
export async function GET() {
  ...
  const items = await prisma.pathbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.pathbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "pathbuilder.create", targetType: "pathbuilder", targetId: item.id,
    },
  });
