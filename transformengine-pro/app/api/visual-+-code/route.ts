...
export async function GET() {
  ...
  const items = await prisma.visual+code.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visual+code.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visual+code.create", targetType: "visual+code", targetId: item.id,
    },
  });
