...
export async function GET() {
  ...
  const items = await prisma.cross-platformlaunch.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cross-platformlaunch.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cross-platformlaunch.create", targetType: "cross-platformlaunch", targetId: item.id,
    },
  });
