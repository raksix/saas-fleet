...
export async function GET() {
  ...
  const items = await prisma.schemaextraction.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.schemaextraction.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "schemaextraction.create", targetType: "schemaextraction", targetId: item.id,
    },
  });
