...
export async function GET() {
  ...
  const items = await prisma.auto-routing.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-routing.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-routing.create", targetType: "auto-routing", targetId: item.id,
    },
  });
