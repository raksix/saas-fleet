...
export async function GET() {
  ...
  const items = await prisma.multi-destination.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-destination.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-destination.create", targetType: "multi-destination", targetId: item.id,
    },
  });
