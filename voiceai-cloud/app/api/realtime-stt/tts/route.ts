...
export async function GET() {
  ...
  const items = await prisma.realtimestt/tts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.realtimestt/tts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "realtimestt/tts.create", targetType: "realtimestt/tts", targetId: item.id,
    },
  });
