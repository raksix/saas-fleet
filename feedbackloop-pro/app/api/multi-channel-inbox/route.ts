...
export async function GET() {
  ...
  const items = await prisma.multi-channelinbox.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-channelinbox.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-channelinbox.create", targetType: "multi-channelinbox", targetId: item.id,
    },
  });
