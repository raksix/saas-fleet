...
export async function GET() {
  ...
  const items = await prisma.multi-platform.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-platform.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-platform.create", targetType: "multi-platform", targetId: item.id,
    },
  });
