...
export async function GET() {
  ...
  const items = await prisma.asyncbydefault.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.asyncbydefault.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "asyncbydefault.create", targetType: "asyncbydefault", targetId: item.id,
    },
  });
