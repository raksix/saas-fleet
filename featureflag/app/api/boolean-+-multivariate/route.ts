...
export async function GET() {
  ...
  const items = await prisma.boolean+multivariate.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.boolean+multivariate.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "boolean+multivariate.create", targetType: "boolean+multivariate", targetId: item.id,
    },
  });
