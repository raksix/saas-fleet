...
export async function GET() {
  ...
  const items = await prisma.doc-to-quizai.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.doc-to-quizai.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "doc-to-quizai.create", targetType: "doc-to-quizai", targetId: item.id,
    },
  });
