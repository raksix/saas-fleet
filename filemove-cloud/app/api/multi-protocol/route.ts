...
export async function GET() {
  ...
  const items = await prisma.multi-protocol.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-protocol.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-protocol.create", targetType: "multi-protocol", targetId: item.id,
    },
  });
