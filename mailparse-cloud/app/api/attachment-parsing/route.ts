...
export async function GET() {
  ...
  const items = await prisma.attachmentparsing.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.attachmentparsing.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "attachmentparsing.create", targetType: "attachmentparsing", targetId: item.id,
    },
  });
