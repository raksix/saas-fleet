...
export async function GET() {
  ...
  const items = await prisma.personageneration.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.personageneration.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "personageneration.create", targetType: "personageneration", targetId: item.id,
    },
  });
