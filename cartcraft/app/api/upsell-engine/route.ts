...
export async function GET() {
  ...
  const items = await prisma.upsellengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.upsellengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "upsellengine.create", targetType: "upsellengine", targetId: item.id,
    },
  });
