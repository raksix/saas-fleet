...
export async function GET() {
  ...
  const items = await prisma.timelineviews.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.timelineviews.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "timelineviews.create", targetType: "timelineviews", targetId: item.id,
    },
  });
