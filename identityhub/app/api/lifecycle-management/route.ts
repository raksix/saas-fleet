...
export async function GET() {
  ...
  const items = await prisma.lifecyclemanagement.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.lifecyclemanagement.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "lifecyclemanagement.create", targetType: "lifecyclemanagement", targetId: item.id,
    },
  });
