...
export async function GET() {
  ...
  const items = await prisma.column-levellineage.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.column-levellineage.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "column-levellineage.create", targetType: "column-levellineage", targetId: item.id,
    },
  });
