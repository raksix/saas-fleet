...
export async function GET() {
  ...
  const items = await prisma.auto-chapter.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-chapter.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-chapter.create", targetType: "auto-chapter", targetId: item.id,
    },
  });
