...
export async function GET() {
  ...
  const items = await prisma.in-productchecklists.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.in-productchecklists.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "in-productchecklists.create", targetType: "in-productchecklists", targetId: item.id,
    },
  });
