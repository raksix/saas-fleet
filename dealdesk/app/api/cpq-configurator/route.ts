...
export async function GET() {
  ...
  const items = await prisma.cpqconfigurator.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cpqconfigurator.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cpqconfigurator.create", targetType: "cpqconfigurator", targetId: item.id,
    },
  });
