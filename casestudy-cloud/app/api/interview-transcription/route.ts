...
export async function GET() {
  ...
  const items = await prisma.interviewtranscription.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.interviewtranscription.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "interviewtranscription.create", targetType: "interviewtranscription", targetId: item.id,
    },
  });
