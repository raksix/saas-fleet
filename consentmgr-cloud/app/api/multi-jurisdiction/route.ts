...
export async function GET() {
  ...
  const items = await prisma.multi-jurisdiction.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-jurisdiction.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-jurisdiction.create", targetType: "multi-jurisdiction", targetId: item.id,
    },
  });
