...
export async function GET() {
  ...
  const items = await prisma.dynamicinsertion.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.dynamicinsertion.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "dynamicinsertion.create", targetType: "dynamicinsertion", targetId: item.id,
    },
  });
