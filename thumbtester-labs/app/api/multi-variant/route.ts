...
export async function GET() {
  ...
  const items = await prisma.multi-variant.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-variant.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-variant.create", targetType: "multi-variant", targetId: item.id,
    },
  });
