...
export async function GET() {
  ...
  const items = await prisma.timingtracker.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.timingtracker.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "timingtracker.create", targetType: "timingtracker", targetId: item.id,
    },
  });
