...
export async function GET() {
  ...
  const items = await prisma.livewhiteboard.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.livewhiteboard.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "livewhiteboard.create", targetType: "livewhiteboard", targetId: item.id,
    },
  });
