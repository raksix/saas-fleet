...
export async function GET() {
  ...
  const items = await prisma.full-textsearch.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.full-textsearch.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "full-textsearch.create", targetType: "full-textsearch", targetId: item.id,
    },
  });
