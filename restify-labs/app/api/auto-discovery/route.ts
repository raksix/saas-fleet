...
export async function GET() {
  ...
  const items = await prisma.auto-discovery.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-discovery.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-discovery.create", targetType: "auto-discovery", targetId: item.id,
    },
  });
