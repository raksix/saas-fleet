...
export async function GET() {
  ...
  const items = await prisma.drag-dropformbuilder.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.drag-dropformbuilder.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "drag-dropformbuilder.create", targetType: "drag-dropformbuilder", targetId: item.id,
    },
  });
