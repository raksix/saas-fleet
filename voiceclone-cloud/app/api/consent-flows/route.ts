...
export async function GET() {
  ...
  const items = await prisma.consentflows.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.consentflows.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "consentflows.create", targetType: "consentflows", targetId: item.id,
    },
  });
