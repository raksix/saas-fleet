...
export async function GET() {
  ...
  const items = await prisma.voicetraining.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.voicetraining.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "voicetraining.create", targetType: "voicetraining", targetId: item.id,
    },
  });
