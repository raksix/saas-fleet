...
export async function GET() {
  ...
  const items = await prisma.cross-platformmetrics.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cross-platformmetrics.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cross-platformmetrics.create", targetType: "cross-platformmetrics", targetId: item.id,
    },
  });
