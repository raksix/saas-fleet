...
export async function GET() {
  ...
  const items = await prisma.variablesubstitution.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.variablesubstitution.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "variablesubstitution.create", targetType: "variablesubstitution", targetId: item.id,
    },
  });
