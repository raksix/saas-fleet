...
export async function GET() {
  ...
  const items = await prisma.airesumescoring.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.airesumescoring.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "airesumescoring.create", targetType: "airesumescoring", targetId: item.id,
    },
  });
