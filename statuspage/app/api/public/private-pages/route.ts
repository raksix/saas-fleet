...
export async function GET() {
  ...
  const items = await prisma.public/privatepages.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.public/privatepages.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "public/privatepages.create", targetType: "public/privatepages", targetId: item.id,
    },
  });
