...
export async function GET() {
  ...
  const items = await prisma.semanticlayer.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.semanticlayer.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "semanticlayer.create", targetType: "semanticlayer", targetId: item.id,
    },
  });
