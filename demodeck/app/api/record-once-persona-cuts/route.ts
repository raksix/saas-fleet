...
export async function GET() {
  ...
  const items = await prisma.record-oncepersonacuts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.record-oncepersonacuts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "record-oncepersonacuts.create", targetType: "record-oncepersonacuts", targetId: item.id,
    },
  });
