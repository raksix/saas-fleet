...
export async function GET() {
  ...
  const items = await prisma.schemafederation.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.schemafederation.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "schemafederation.create", targetType: "schemafederation", targetId: item.id,
    },
  });
