...
export async function GET() {
  ...
  const items = await prisma.deliverabletracking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.deliverabletracking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "deliverabletracking.create", targetType: "deliverabletracking", targetId: item.id,
    },
  });
