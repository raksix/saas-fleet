...
export async function GET() {
  ...
  const items = await prisma.url-to-video.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.url-to-video.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "url-to-video.create", targetType: "url-to-video", targetId: item.id,
    },
  });
