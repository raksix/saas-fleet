...
export async function GET() {
  ...
  const items = await prisma.plain-englishmetrics.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.plain-englishmetrics.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "plain-englishmetrics.create", targetType: "plain-englishmetrics", targetId: item.id,
    },
  });
