...
export async function GET() {
  ...
  const items = await prisma.auto-framing.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-framing.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-framing.create", targetType: "auto-framing", targetId: item.id,
    },
  });
