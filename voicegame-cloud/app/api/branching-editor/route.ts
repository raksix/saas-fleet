...
export async function GET() {
  ...
  const items = await prisma.branchingeditor.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.branchingeditor.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "branchingeditor.create", targetType: "branchingeditor", targetId: item.id,
    },
  });
