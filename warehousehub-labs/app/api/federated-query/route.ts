...
export async function GET() {
  ...
  const items = await prisma.federatedquery.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.federatedquery.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "federatedquery.create", targetType: "federatedquery", targetId: item.id,
    },
  });
