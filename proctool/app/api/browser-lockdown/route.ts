...
export async function GET() {
  ...
  const items = await prisma.browserlockdown.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.browserlockdown.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "browserlockdown.create", targetType: "browserlockdown", targetId: item.id,
    },
  });
