...
export async function GET() {
  ...
  const items = await prisma.templatelibrary.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.templatelibrary.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "templatelibrary.create", targetType: "templatelibrary", targetId: item.id,
    },
  });
