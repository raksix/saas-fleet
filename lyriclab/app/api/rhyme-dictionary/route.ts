...
export async function GET() {
  ...
  const items = await prisma.rhymedictionary.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.rhymedictionary.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "rhymedictionary.create", targetType: "rhymedictionary", targetId: item.id,
    },
  });
