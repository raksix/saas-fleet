...
export async function GET() {
  ...
  const items = await prisma.fsrsscheduler.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.fsrsscheduler.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "fsrsscheduler.create", targetType: "fsrsscheduler", targetId: item.id,
    },
  });
