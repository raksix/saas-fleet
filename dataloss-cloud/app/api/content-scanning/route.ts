...
export async function GET() {
  ...
  const items = await prisma.contentscanning.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.contentscanning.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "contentscanning.create", targetType: "contentscanning", targetId: item.id,
    },
  });
