...
export async function GET() {
  ...
  const items = await prisma.multi-channel.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-channel.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-channel.create", targetType: "multi-channel", targetId: item.id,
    },
  });
