...
export async function GET() {
  ...
  const items = await prisma.versionhistory.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.versionhistory.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "versionhistory.create", targetType: "versionhistory", targetId: item.id,
    },
  });
