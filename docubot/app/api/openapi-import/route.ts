...
export async function GET() {
  ...
  const items = await prisma.openapiimport.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.openapiimport.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "openapiimport.create", targetType: "openapiimport", targetId: item.id,
    },
  });
