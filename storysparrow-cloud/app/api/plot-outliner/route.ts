...
export async function GET() {
  ...
  const items = await prisma.plotoutliner.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.plotoutliner.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "plotoutliner.create", targetType: "plotoutliner", targetId: item.id,
    },
  });
