...
export async function GET() {
  ...
  const items = await prisma.policyascode.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.policyascode.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "policyascode.create", targetType: "policyascode", targetId: item.id,
    },
  });
