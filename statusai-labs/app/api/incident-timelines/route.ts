...
export async function GET() {
  ...
  const items = await prisma.incidenttimelines.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.incidenttimelines.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "incidenttimelines.create", targetType: "incidenttimelines", targetId: item.id,
    },
  });
