...
export async function GET() {
  ...
  const items = await prisma.pipelinekanban.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.pipelinekanban.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "pipelinekanban.create", targetType: "pipelinekanban", targetId: item.id,
    },
  });
