...
export async function GET() {
  ...
  const items = await prisma.photo/videoreviews.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.photo/videoreviews.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "photo/videoreviews.create", targetType: "photo/videoreviews", targetId: item.id,
    },
  });
