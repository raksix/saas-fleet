...
export async function GET() {
  ...
  const items = await prisma.plan/prorationengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.plan/prorationengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "plan/prorationengine.create", targetType: "plan/prorationengine", targetId: item.id,
    },
  });
