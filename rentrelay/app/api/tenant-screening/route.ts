...
export async function GET() {
  ...
  const items = await prisma.tenantscreening.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.tenantscreening.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "tenantscreening.create", targetType: "tenantscreening", targetId: item.id,
    },
  });
