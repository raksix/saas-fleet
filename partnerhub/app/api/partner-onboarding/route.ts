...
export async function GET() {
  ...
  const items = await prisma.partneronboarding.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.partneronboarding.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "partneronboarding.create", targetType: "partneronboarding", targetId: item.id,
    },
  });
