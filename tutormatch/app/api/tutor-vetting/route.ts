...
export async function GET() {
  ...
  const items = await prisma.tutorvetting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.tutorvetting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "tutorvetting.create", targetType: "tutorvetting", targetId: item.id,
    },
  });
