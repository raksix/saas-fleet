...
export async function GET() {
  ...
  const items = await prisma.documentingestion.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.documentingestion.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "documentingestion.create", targetType: "documentingestion", targetId: item.id,
    },
  });
