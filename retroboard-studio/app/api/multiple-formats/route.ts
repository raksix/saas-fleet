...
export async function GET() {
  ...
  const items = await prisma.multipleformats.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multipleformats.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multipleformats.create", targetType: "multipleformats", targetId: item.id,
    },
  });
