...
export async function GET() {
  ...
  const items = await prisma.cross-teamviews.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cross-teamviews.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cross-teamviews.create", targetType: "cross-teamviews", targetId: item.id,
    },
  });
