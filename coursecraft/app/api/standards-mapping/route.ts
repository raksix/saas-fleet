...
export async function GET() {
  ...
  const items = await prisma.standardsmapping.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.standardsmapping.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "standardsmapping.create", targetType: "standardsmapping", targetId: item.id,
    },
  });
