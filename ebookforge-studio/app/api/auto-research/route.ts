...
export async function GET() {
  ...
  const items = await prisma.auto-research.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-research.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-research.create", targetType: "auto-research", targetId: item.id,
    },
  });
