...
export async function GET() {
  ...
  const items = await prisma.agentco-pilot.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.agentco-pilot.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "agentco-pilot.create", targetType: "agentco-pilot", targetId: item.id,
    },
  });
