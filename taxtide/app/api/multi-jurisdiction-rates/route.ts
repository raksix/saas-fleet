...
export async function GET() {
  ...
  const items = await prisma.multi-jurisdictionrates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-jurisdictionrates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-jurisdictionrates.create", targetType: "multi-jurisdictionrates", targetId: item.id,
    },
  });
