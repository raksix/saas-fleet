...
export async function GET() {
  ...
  const items = await prisma.smarttriggers.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.smarttriggers.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "smarttriggers.create", targetType: "smarttriggers", targetId: item.id,
    },
  });
