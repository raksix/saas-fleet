...
export async function GET() {
  ...
  const items = await prisma.visualdesigner.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualdesigner.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualdesigner.create", targetType: "visualdesigner", targetId: item.id,
    },
  });
