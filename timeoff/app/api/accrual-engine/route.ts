...
export async function GET() {
  ...
  const items = await prisma.accrualengine.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.accrualengine.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "accrualengine.create", targetType: "accrualengine", targetId: item.id,
    },
  });
