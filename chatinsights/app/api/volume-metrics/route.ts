...
export async function GET() {
  ...
  const items = await prisma.volumemetrics.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.volumemetrics.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "volumemetrics.create", targetType: "volumemetrics", targetId: item.id,
    },
  });
