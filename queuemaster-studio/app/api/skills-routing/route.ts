...
export async function GET() {
  ...
  const items = await prisma.skillsrouting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.skillsrouting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "skillsrouting.create", targetType: "skillsrouting", targetId: item.id,
    },
  });
