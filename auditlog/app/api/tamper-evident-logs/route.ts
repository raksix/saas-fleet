...
export async function GET() {
  ...
  const items = await prisma.tamper-evidentlogs.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.tamper-evidentlogs.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "tamper-evidentlogs.create", targetType: "tamper-evidentlogs", targetId: item.id,
    },
  });
