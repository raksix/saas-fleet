...
export async function GET() {
  ...
  const items = await prisma.conflictresolution.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.conflictresolution.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "conflictresolution.create", targetType: "conflictresolution", targetId: item.id,
    },
  });
