...
export async function GET() {
  ...
  const items = await prisma.smartmatching.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.smartmatching.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "smartmatching.create", targetType: "smartmatching", targetId: item.id,
    },
  });
