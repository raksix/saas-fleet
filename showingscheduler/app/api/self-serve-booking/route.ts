...
export async function GET() {
  ...
  const items = await prisma.self-servebooking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.self-servebooking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "self-servebooking.create", targetType: "self-servebooking", targetId: item.id,
    },
  });
