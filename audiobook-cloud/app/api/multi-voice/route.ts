...
export async function GET() {
  ...
  const items = await prisma.multi-voice.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-voice.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-voice.create", targetType: "multi-voice", targetId: item.id,
    },
  });
