...
export async function GET() {
  ...
  const items = await prisma.departmenthierarchies.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.departmenthierarchies.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "departmenthierarchies.create", targetType: "departmenthierarchies", targetId: item.id,
    },
  });
