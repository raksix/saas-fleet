...
export async function GET() {
  ...
  const items = await prisma.episodeoutlines.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.episodeoutlines.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "episodeoutlines.create", targetType: "episodeoutlines", targetId: item.id,
    },
  });
