...
export async function GET() {
  ...
  const items = await prisma.behavioraldetection.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.behavioraldetection.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "behavioraldetection.create", targetType: "behavioraldetection", targetId: item.id,
    },
  });
