...
export async function GET() {
  ...
  const items = await prisma.multi-sourceingest.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-sourceingest.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-sourceingest.create", targetType: "multi-sourceingest", targetId: item.id,
    },
  });
