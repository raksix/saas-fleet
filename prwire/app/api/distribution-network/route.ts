...
export async function GET() {
  ...
  const items = await prisma.distributionnetwork.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.distributionnetwork.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "distributionnetwork.create", targetType: "distributionnetwork", targetId: item.id,
    },
  });
