...
export async function GET() {
  ...
  const items = await prisma.knowledgetraining.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.knowledgetraining.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "knowledgetraining.create", targetType: "knowledgetraining", targetId: item.id,
    },
  });
