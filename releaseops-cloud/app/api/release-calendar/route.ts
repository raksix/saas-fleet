...
export async function GET() {
  ...
  const items = await prisma.releasecalendar.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.releasecalendar.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "releasecalendar.create", targetType: "releasecalendar", targetId: item.id,
    },
  });
