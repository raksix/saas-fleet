...
export async function GET() {
  ...
  const items = await prisma.epcse-prescribing.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.epcse-prescribing.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "epcse-prescribing.create", targetType: "epcse-prescribing", targetId: item.id,
    },
  });
