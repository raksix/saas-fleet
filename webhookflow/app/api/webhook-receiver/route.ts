...
export async function GET() {
  ...
  const items = await prisma.webhookreceiver.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.webhookreceiver.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "webhookreceiver.create", targetType: "webhookreceiver", targetId: item.id,
    },
  });
