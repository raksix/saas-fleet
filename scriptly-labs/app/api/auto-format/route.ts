...
export async function GET() {
  ...
  const items = await prisma.auto-format.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-format.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-format.create", targetType: "auto-format", targetId: item.id,
    },
  });
