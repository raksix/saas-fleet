...
export async function GET() {
  ...
  const items = await prisma.pulsesurveys.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.pulsesurveys.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "pulsesurveys.create", targetType: "pulsesurveys", targetId: item.id,
    },
  });
