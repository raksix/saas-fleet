...
export async function GET() {
  ...
  const items = await prisma.scopedriftalerts.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.scopedriftalerts.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "scopedriftalerts.create", targetType: "scopedriftalerts", targetId: item.id,
    },
  });
