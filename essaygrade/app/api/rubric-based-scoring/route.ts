...
export async function GET() {
  ...
  const items = await prisma.rubric-basedscoring.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.rubric-basedscoring.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "rubric-basedscoring.create", targetType: "rubric-basedscoring", targetId: item.id,
    },
  });
