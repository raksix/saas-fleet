...
export async function GET() {
  ...
  const items = await prisma.pomodoro+flowtracking.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.pomodoro+flowtracking.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "pomodoro+flowtracking.create", targetType: "pomodoro+flowtracking", targetId: item.id,
    },
  });
