...
export async function GET() {
  ...
  const items = await prisma.visualvarianteditor.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualvarianteditor.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualvarianteditor.create", targetType: "visualvarianteditor", targetId: item.id,
    },
  });
