...
export async function GET() {
  ...
  const items = await prisma.visualeditor.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.visualeditor.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "visualeditor.create", targetType: "visualeditor", targetId: item.id,
    },
  });
