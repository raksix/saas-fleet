...
export async function GET() {
  ...
  const items = await prisma.noiseremoval.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.noiseremoval.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "noiseremoval.create", targetType: "noiseremoval", targetId: item.id,
    },
  });
