...
export async function GET() {
  ...
  const items = await prisma.formdesigner.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.formdesigner.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "formdesigner.create", targetType: "formdesigner", targetId: item.id,
    },
  });
