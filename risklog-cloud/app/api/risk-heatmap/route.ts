...
export async function GET() {
  ...
  const items = await prisma.riskheatmap.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.riskheatmap.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "riskheatmap.create", targetType: "riskheatmap", targetId: item.id,
    },
  });
