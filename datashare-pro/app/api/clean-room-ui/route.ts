...
export async function GET() {
  ...
  const items = await prisma.cleanroomui.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.cleanroomui.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "cleanroomui.create", targetType: "cleanroomui", targetId: item.id,
    },
  });
