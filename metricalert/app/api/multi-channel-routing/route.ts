...
export async function GET() {
  ...
  const items = await prisma.multi-channelrouting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-channelrouting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-channelrouting.create", targetType: "multi-channelrouting", targetId: item.id,
    },
  });
