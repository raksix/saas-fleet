...
export async function GET() {
  ...
  const items = await prisma.arr/churndashboard.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.arr/churndashboard.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "arr/churndashboard.create", targetType: "arr/churndashboard", targetId: item.id,
    },
  });
