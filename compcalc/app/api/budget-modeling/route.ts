...
export async function GET() {
  ...
  const items = await prisma.budgetmodeling.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.budgetmodeling.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "budgetmodeling.create", targetType: "budgetmodeling", targetId: item.id,
    },
  });
