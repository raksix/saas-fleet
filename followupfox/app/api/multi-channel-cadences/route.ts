...
export async function GET() {
  ...
  const items = await prisma.multi-channelcadences.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.multi-channelcadences.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "multi-channelcadences.create", targetType: "multi-channelcadences", targetId: item.id,
    },
  });
