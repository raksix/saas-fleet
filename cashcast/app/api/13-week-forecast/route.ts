...
export async function GET() {
  ...
  const items = await prisma.13-weekforecast.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.13-weekforecast.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "13-weekforecast.create", targetType: "13-weekforecast", targetId: item.id,
    },
  });
