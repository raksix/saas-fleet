...
export async function GET() {
  ...
  const items = await prisma.kickofftemplates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.kickofftemplates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "kickofftemplates.create", targetType: "kickofftemplates", targetId: item.id,
    },
  });
