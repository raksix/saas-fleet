...
export async function GET() {
  ...
  const items = await prisma.voicecapture(mobile).findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.voicecapture(mobile).create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "voicecapture(mobile).create", targetType: "voicecapture(mobile)", targetId: item.id,
    },
  });
