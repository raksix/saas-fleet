...
export async function GET() {
  ...
  const items = await prisma.demandforecasting.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.demandforecasting.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "demandforecasting.create", targetType: "demandforecasting", targetId: item.id,
    },
  });
