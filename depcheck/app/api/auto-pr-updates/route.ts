...
export async function GET() {
  ...
  const items = await prisma.auto-prupdates.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.auto-prupdates.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "auto-prupdates.create", targetType: "auto-prupdates", targetId: item.id,
    },
  });
