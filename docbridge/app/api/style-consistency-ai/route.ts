...
export async function GET() {
  ...
  const items = await prisma.styleconsistencyai.findMany({
...
export async function POST(req: Request) {
  ...
  const item = await prisma.styleconsistencyai.create({
    data: { orgId, name: parsed.data.name },
  });
  await prisma.auditLog.create({
    data: {
      orgId, userId: (session.user as any).id,
      action: "styleconsistencyai.create", targetType: "styleconsistencyai", targetId: item.id,
    },
  });
